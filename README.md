...
installs it to `~/.config/omarchy/plugins/omarchy.tide/`, and places it in
the center of the bar. The bar hot-reloads; no shell restart is needed.

Not on the bar after all? Enable it manually:

```sh
omarchy plugin enable omarchy.tide --section center
```
...
- Data is cached to `~/.local/state/omarchy/settings/omarchy-tide-<location>-*.json`
  and the location to `omarchy-tide-location.json` for offline use.
